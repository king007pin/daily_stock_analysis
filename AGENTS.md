...
## 1.3 严格数据真实性与防幻觉底线 (Zero-Hallucination Invariant)

- **禁止捏造或臆测任何行情数据**：包括且不限于当前股价、涨跌幅、最高/最低价、市盈率 (P/E)、成交量、市值、支撑阻力位、财务指标。
- **强制真实数据源锚定**：所有股票分析、策略制定、仓位计算与报告输出前，必须先通过真实数据源（`scrape_live_web_quotes`、`LiveWebScraperService`、`DataFetcher` 或已验证 OHLCV DataFrame）获取数据，严禁仅凭模型先验知识或模糊检索记忆生成具体数值。
- **显式标明数据源与时效**：输出的每个价格必须带有数据源标记（例如 `[Screener.in Live]`、`[Google Finance]` 或具体数据时间戳）。
- **缺失数据主动报错/降级**：若无法通过真实数据源抓取到有效行情，必须显式返回数据缺失错误，严禁以“合理估测”填充数字。
