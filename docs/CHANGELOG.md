...
- [修复] `06-Scripts-Bridge/run_intraday_live_scanner.py`（Obsidian vault 侧脚本）此前对 prices_1m/volumes_1m/RSI/volume_surge_ratio/circuit_buffer_pct/is_fno_eligible/has_news_catalyst 等字段使用基于 live_price 的合成/硬编码值，违反 AGENTS.md §1.3 防幻觉底线；改为真实数据：新增 `src/services/intraday_bar_fetcher.py`（真实 5m K 线、RSI、成交量激增比、F&O 名单查询）与 `src/services/intraday_catalyst_check.py`（复用现有 SearchService 判断真实新闻催化剂），并将熔断缓冲计算提取到 `src/utils/market_math.py::compute_circuit_buffer_pct` 供 kronos_service 与该脚本共用。无法获取真实数据的标的会被跳过并在报告中列出，不再回退到合成数值。
- [修复] `06-Scripts-Bridge/run_eod_pipeline.py` 与 `EODPipelineService` 模板此前对 Nifty/Sensex 收盘价、涨跌幅、FII/DII 资金流、top_gainers 使用硬编码常量，且模板自身还内嵌了与输入无关的固定文案（`+0.42%`、`Risk-On`、固定 BTST 持仓篮子），`btst_performance_summary` 字段从未在模板中被实际使用；同样违反 AGENTS.md §1.3。改为真实数据：新增 `src/services/eod_market_data.py`（真实指数收盘/涨跌幅、基于自身涨跌方向推导的 Risk-On/Risk-Off、真实 watchlist 涨跌幅排名、基于 `DecisionSignalOutcomeService` 的真实 BTST（horizon=1d）信号统计），`src/services/vault_watchlist_loader.py` 抽取为 intraday 与 EOD 两个桥接脚本共用的真实 universe 加载器。FII/DII 真实数据源调研后仍未接入（moneycontrol/groww 页面为客户端渲染 SPA，无头浏览器抓取首次尝试超过 60 秒未完成）——诚实降级为 `fii_dii_status="UNAVAILABLE"` 并附真实原因，不用编造数字填充，作为后续待办跟踪。
- [新功能] 新增 `src/services/nse_universe_discovery.py`，基于 `jugaad_data.nse.NSELive.top_stocks()`（NSE 官方实时快照接口，已在 `data_provider/jugaad_fetcher.py` 中被信任用于盘口深度数据）实现真实的 NSE 全市场发现能力，弥补此前系统只能看到 18 个手工维护 watchlist 标的的盲区；新增 `06-Scripts-Bridge/run_nse_discovery_scan.py`（Obsidian vault 侧脚本，写入 `02-Reports/NSE-Discovery/` 真实涨跌幅/活跃度快照）；`run_intraday_live_scanner.py` universe 从纯 watchlist 扩展为 watchlist + 每类别 top 10 真实发现标的（并集去重，发现失败时降级为仅 watchlist、不阻断运行），报告新增 Source 列标注标的来源。实盘验证：发现的标的中包含此前系统完全未能识别的真实异动股，并被正确判定为可交易信号。
- [新功能] 新增 `src/services/bhavcopy_reconciliation_service.py`：用 NSE 官方 bhavcopy 对账已存的 `stock_daily` 日线，成交量（免复权，可直接比对）与收盘价（复用 bhavcopy 客户端的容差口径，客户端未提供该 helper 时价格比对显式标记为 unavailable，只比成交量，不假装比过）不一致的写入隔离表留证，永远不覆盖已存 K 线；一致的回填缺失的 `delivery_qty` / `delivery_pct`（已有值不覆盖）。同一交易日重复运行不会重复写隔离记录。周末、节假日、NSE 故障导致的 bhavcopy 缺失属正常结果，只在汇总里说明，不抛错也不补数。新增开关 `BHAVCOPY_RECONCILIATION_ENABLED`，默认关闭（需联网），与 `DECISION_OUTCOME_DAILY_REFILL_ENABLED` 同为 .env 级运维开关，不进 Web 设置页。
- [新功能] 新增 Telegram 机器人双向交互适配器与 Long-Polling 客户端，支持群聊/私聊 /analyze、/ask、/market 等命令交互、用户白名单、Webhook 验签及免公网 IP 本地轮询模式。
- [新功能] 印度 NSE（`.NS`）日线新增 `JugaadDataFetcher` 兜底数据源（Priority 6，无需凭据，数据来自 NSE bhavcopy），在 `YfinanceFetcher` 失败时自动降级；BSE（`.BO`）暂不覆盖，仍仅 `YfinanceFetcher`。
- [修复] 印度（`in`）市场此前未加入 `VALID_MARKETS`，导致每一次 NSE/BSE 个股分析都会在决策信号提取阶段被静默跳过（报告本身正常生成，但从不产出可回测/校准的决策信号）；现已修复，`in` 同步加入 `PARTIAL_VALUATION_MARKETS`（对齐 jp/kr/tw 的数据覆盖现状）与组合默认币种 INR。
- [修复] `horizon="intraday"` 此前未在 `SUPPORTED_OUTCOME_HORIZONS` 中登记，所有日内/当日交易信号的结果评估一律返回 `unsupported_horizon`，永远不产生校准数据；现已支持，评估口径为信号锚定日当天自身 K 线的开盘到收盘同日近似（同一根日线内止损/止盈先后顺序仍不可判定，沿用既有保守假设）。
- [新功能] 印度（NSE）实时行情新增盘口深度来源 `JugaadDataFetcher.get_realtime_quote()`（基于 `NSELive`），接入 `DataFetcherManager` 实时行情兜底链：`YfinanceFetcher` 成功时补充买一/卖一价量，失败时作为唯一兜底源——此前印股实时行情零兜底、无盘口深度字段。`UnifiedRealtimeQuote` 新增 `bid_price`/`bid_qty`/`ask_price`/`ask_qty` 字段（其余 8 个已有数据源该字段默认为空，向后兼容）。
- [新功能] 分析报告 `intelligence` 区块新增 `news_sentiment_score`（0-100，与既有整体 `sentiment_score` 区分，专指新闻/舆情本身的情绪极性），复用现有单股 LLM 调用产出，不新增依赖或 API 调用。
- [修复] 资金流校准此前对所有市场一律生效，但资金流是 A 股专属数据源（非 CN 市场经 `_build_offshore_fundamental_context` 时该区块被有意跳过，永久返回不可用）。结果是每一次印股/美股分析的 buy 结论都会被降级为「持有观察」后才落库，决策信号中 48 条有 43 条 action 为 `watch`，而 `watch` 不可评估，校准数据长期为空。现改为按市场判定：仅 CN 走原有降级逻辑（行为完全不变），其余市场记录「该市场不适用资金流数据」并保留模型原始方向性结论。
- [修复] `not_up`（sell/reduce/avoid）方向此前没有中性区间，凡涨幅未超过阈值一律记为 hit——实测 1476 根真实日线下防守型结论命中率被抬高到约 89%；现与 `not_down` 对称：涨幅 ≤0 记 hit，≥阈值记 miss，中间记 neutral。同时 `_classify_outcome` 补齐了缺失的 `not_up` 分支（此前会错误落入 `flat` 分支，与 `_classify_signal_outcome` 结论不一致）。
- [改进] 决策信号结果评估的中性区间阈值改为从配置读取并支持按 horizon 覆盖（新增 `BACKTEST_NEUTRAL_BAND_PCT_BY_HORIZON`），此前该处硬编码为 2.0 且忽略已有的 `BACKTEST_NEUTRAL_BAND_PCT`；不配置时行为不变。
- [改进] `DECISION_SIGNAL_OUTCOME_ENGINE_VERSION` 升级为 `decision-signal-v2`：上述评估口径变更使旧结果不可与新结果混用，旧版本行记录会在下次运行时自动重算。
- [改进] 决策动作到方向的映射改为显式的 `DIRECTION_BY_ACTION` 与 `NON_DIRECTIONAL_ACTIONS` 两张表，并新增测试断言 `DecisionAction` 的每个取值都被明确归类，避免新增动作被静默视为不可评估；最大不利偏移的多空侧归类同步提取为具名常量 `LONG_SIDE_EXCURSION_ACTIONS`/`SHORT_SIDE_EXCURSION_ACTIONS`（与方向映射有意不同：偏移是描述性统计，不参与胜率计算）。
- [新功能] 新增 `BACKTEST_BAR_ADAPTIVE_HIGH_LOW_ORDERING`（默认 false，行为不变）：同一根日线内同时触及止损与止盈时，可选择按「先到达距离开盘价更近的一端」解析先后顺序，first_hit 输出 take_profit/stop_loss、exit_reason 输出 adaptive_*；开盘价缺失或恰好等距时回退到原有的保守止损优先假设。这是确定性启发式，不还原真实成交路径，技术来源为 NautilusTrader 的 `bar_adaptive_high_low_ordering` 场馆选项。`DailyBarLike` 协议相应新增可选 `open` 字段（纯增量，既有数据源不受影响）。切换该开关会改变已存结果口径，需同步提升 `BACKTEST_ENGINE_VERSION`。
- [新功能] 新增 `src/services/transaction_cost_service.py`：印度股票交易成本模型（经纪佣金及封顶、STT 区分日内/交割与买/卖、交易所交易费、SEBI 周转费、印花税仅买入方、GST 作用于费用基数），提供单笔成本明细、往返成本与 `breakeven_move_pct`（保本所需价格变动百分比）。所有费率默认 `None` 且未配置时显式报错，绝不按 0 计算——凭记忆填写的税率等同于捏造（AGENTS.md §1.3）；模块内 `RATE_SOURCES` 列明每项费率的取数权威来源。新增 `TXN_COST_*` 共 10 项配置。
- [新功能] 新增 `src/services/benchmark_return_service.py`：基准相对（超额）收益计算，复用 `MarketProfile.mood_index_code` 与 `DataFetcherManager.get_daily_data`，`in` 默认基准为 Nifty 50（`^NSEI`）、`us` 为 `^GSPC`、`cn` 为 `sh000001`；未配置基准的市场返回显式 `no_benchmark_configured`，基准数据缺失时返回 `None` + 原因，**绝不以 0 代替缺失**（否则等于把「未知」谎报为「大盘持平」）。同时提供与日内信号同口径的当日开盘→收盘基准变体。
- [新功能] 新增 `src/services/risk_limit_service.py`：组合级风险限额（最大回撤熔断、单笔风险占权益比例、单日亏损上限、总敞口上限、持仓数量、单行业持仓数、相关性簇上限）。全部 `None` 时不改变任何现有行为；单笔风险按 |入场价-止损价|×数量/权益 计算，未设止损在该限额开启时判定为违规；整体 fail-closed，任何缺失输入或内部异常一律返回 `allowed=False` 并给出原因。新增 `RISK_LIMIT_*` 共 8 项配置。
- [新功能] 新增 `src/services/performance_stats_service.py`：诚实统计口径。Sharpe 采用 bootstrap 置信区间（不假设正态，可复现种子），胜率采用 Wilson 区间（小样本正确），二者均设最小样本门槛（Sharpe N≥30、胜率 N≥20），低于门槛返回 `insufficient_sample` 且点估计为 `None` 而非「带注释的数字」；`summary()` 强制同时输出 N、`unable` 原因分布与 neutral 占比，缺任一键即抛错，从结构上杜绝「裸胜率百分比」。另提供 `deflated_sharpe_ratio`，并在文档中明确其仅适用于真实执行过的变体回测，不得用于从未执行的策略文档。
- [修复] 决策信号结果评估此前无法自愈缺失的日线数据：`BacktestService` 自其编写起即有 `_try_fill_daily_data`，而该路径没有对应机制，导致调度任务停摆时 `stock_daily` 同步停止更新、已记录信号永远无法成熟（2026-08-24 实测确认，需手工补数才产生首批评分结果）。现补齐同名机制并复用相同抓取链；新增 `DECISION_OUTCOME_DAILY_REFILL_ENABLED`，**默认关闭**（补数会发起网络请求，默认开启会让离线测试套件联网——首版实现即因此使 5 个既有用例真实访问 eastmoney 并将该套件从数秒拖到 167 秒）。
- [新功能] 新增 `scripts/screen_and_analyze_eod.py`：EOD 定时任务在个股分析前先跑一次 CN 市场量化选股（`use_llm=False`，仅本地统计打分，不产生额外 LLM 调用），将命中的候选代码与 `STOCK_LIST` 去重合并后一起送入分析流水线。选股引擎（`src/services/screening/`）目前实际仅覆盖 A 股：内置的 10 个策略 YAML 均声明 `market_scope: [cn]`，无一支持 `us`；印度（`in`）市场不在 `pipeline.screen()` 的校验白名单内。因此该功能仅扩充 A 股候选池，不扩充美股或印度股票池——日内交易（NSE）候选仍完全依赖人工维护的观察名单（见 `06-Scripts-Bridge` 早盘任务）。
